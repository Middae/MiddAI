...
DEFAULT_SETTINGS = {
    ...
    "depth": {
        ...
        "instant": {
            ...
            "style_instruction": "Instant mode: answer in one complete sentence, maximum 35 words. No intro, no examples, no sections, and no follow-up question unless the user asks for one. Any assistant personality must fit inside that one sentence.",
        },
        "quick": {
            "max_results": 8,
            "max_good_sources": 2,
...
LEGACY_DEPTH_TOKEN_UPGRADES = {
    "instant": (90, DEFAULT_SETTINGS["depth"]["instant"]["max_answer_tokens"]),
}
...
def upgrade_legacy_depth_settings(user_settings):
    ...
    for preset_name, preset_settings in depth_settings.items():
        ...
        if token_upgrade and preset_settings.get("max_answer_tokens") == token_upgrade[0]:
