...
def format_previous_chats(previous_chats):
    ...
    for number, chat in enumerate(previous_chats, start=1):
        ...
        messages = format_messages(chat.get("messages", []))
        sections.append(
            f"""Previous chat {number} ended at {ended_at}:
{messages}"""
        )
...
- Use selected chat memory only for conversation continuity.
...
- Previous chat context is included only when the user explicitly asks about earlier conversations; otherwise treat it as unavailable.
