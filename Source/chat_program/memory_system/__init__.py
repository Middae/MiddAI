...
def apply_ai_memory_refinement(content, metadata=None):
    memories = judge_memories_from_user_message(content, allow_fallback=False)

    if not memories:
        append_memory_event(
            "ai_judge_refinement",
            content=content,
            status="no_refinements",
            metadata=metadata,
        )
        return []
...
def add_candidates_from_user_content(
    ...
    if use_ai_judge:
        judged_memories = judge_memories_from_user_message(content)
