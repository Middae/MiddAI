...
def load_active_model_with_context(active_model, context_length):
    ...
    gpu_offload = str(active_model.get("gpu_offload") or "off").strip()
...
def runtime_settings_payload():
    ...
    return {
        ...
        "can_update_context": bool(active_model.get("model_key"))
        and str(active_model.get("chat_model_id") or "local-model") == "local-model",
        "memory_mode": active_model.get("memory_mode", ""),
...
def select_assistant():
    ...
    assistant = set_active_assistant(assistant_id)
...
def delete_assistant():
    ...
    try:
        active_assistant = delete_custom_assistant(data.get("assistant_id"))
    except ValueError as error:
        return jsonify({"error": str(error)}), 400
...
def chat_stream():
    ...
    def generate():
        ...
        try:
            ...
            if search_route.intent == SEARCH_INTENT_IMAGE:
                ...
                yield stream_event("status", message="Searching images")
                ...
                try:
                    ...
                    yield stream_event(
                        "error",
                        error=(
                            "Image search failed. Check your internet connection or "
                ...
                if not images:
                    yield stream_event("error", error="No usable image results found.")
                    return
            ...
            images = []
            print("Image search skipped for text search.")
...
def chat():
    ...
    if search_route.intent == SEARCH_INTENT_IMAGE:
        ...
        if not images:
            return jsonify({"error": "No usable image results found."}), 404
